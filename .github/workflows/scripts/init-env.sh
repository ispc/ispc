...
case "$OS" in
    Linux*)
      if [ "$ARCH" == "aarch64" ]; then
        LLVM_TAR=${LLVM_TAR:-"llvm-22.1.6-ubuntu22.04aarch64-Release+Asserts-x86.arm.wasm.tar.xz"}
      else
        LLVM_TAR=${LLVM_TAR:-"llvm-22.1.6-ubuntu22.04-Release+Asserts-x86.arm.wasm.tar.xz"}
      fi
      ;;
    Darwin*)
      LLVM_TAR=${LLVM_TAR:-"llvm-22.1.6-macos-Release+Asserts-universal-x86.arm.wasm.tar.xz"}
      ;;
    *)
