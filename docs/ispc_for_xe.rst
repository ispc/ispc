...
* ``ISPCRT_GPU_DRIVER`` - if more than one supported GPUs are present in the
  system they may be managed by several GPU drivers. The user can the GPU driver
  to be used by the ``ISPCRT`` using ``ISPCRT_GPU_DRIVER`` variable. It should
  be set to a number of drivers as enumerated by the Level Zero runtime. For
  example, in a system with two GPU drivers present, the variable can be set
  to ``0`` or ``1``.
