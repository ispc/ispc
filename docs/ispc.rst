...
target version for macOS and iOS applications which is required by new linker
introduced in Xcode 15.0.
...
The ``--darwin-version-min`` option was added to specify the minimum deployment
target version for macOS and iOS applications which is required by new linker
introduced in Xcode 15.0. The version should be specified in the format ``[major.minor]``.
When an empty string ("") is passed, no minimum version will be specified in
the output binary.
