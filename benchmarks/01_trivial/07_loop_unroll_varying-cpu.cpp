...
static Docs docs(
    "Check the performance and functionality of loop unrolling with varying types.\n"
...
template<typename T>
void init_src(T *buf, size_t num_elems)
{
...
template<typename T>
void init_dst(T *buf, size_t num_elems)
{
    for (int i = 0; i < static_cast<int>(num_elems); i++) {
...
void check(T *src, T *dst, size_t num_elems)
{
    ...
    for (int i = 0; i < static_cast<int>(num_elems); i++) {
        if (buf[i] != dst[i]) { //|| !within_error<T>(buf, dst, i)) {
