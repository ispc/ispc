...
function(builtin_to_cpp bit os_name arch supported_archs supported_oses resultFileName)
    ...
    if ((    ${os_name} STREQUAL "web" AND NOT ${arch} STREQUAL "wasm${bit}") OR
        (NOT ${os_name} STREQUAL "web" AND     ${arch} MATCHES "wasm(32|64)"))
        return()
    endif()
    ...
    elseif ("${bit}" STREQUAL "64" AND ${arch} STREQUAL "wasm64")
    ...
    if (${os_name} STREQUAL "web")
        ...
        add_custom_command(
            OUTPUT ${output}
            COMMAND ${EMCC_EXECUTABLE} -DWASM -s WASM_OBJECT_FILES=0 ${emcc_flags} ${LLVM_TOOLS_OPAQUE_FLAGS} -I${CMAKE_SOURCE_DIR} -c ${inputFilePath} --std=gnu++17 -emit-llvm -c -o -
...
function (generate_common_builtins resultList)
    ...
    if (WASM_ENABLED)
        list (APPEND supported_archs "wasm32" "wasm64")
    ...
    foreach (bit 32 64)
        foreach (os_name "windows" "linux" "freebsd" "macos" "android" "ios" "ps4" "web")
            foreach (arch "x86" "arm" "wasm32" "wasm64")
