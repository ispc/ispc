...
function(dispatch_ll_to_cpp filename os)
    set(input builtins/${filename}.ll)
    set(name builtins-${filename})
    set(cpp ${CMAKE_CURRENT_BINARY_DIR}/${CMAKE_CFG_INTDIR}/${name}.cpp)
    set(bc ${BITCODE_FOLDER}/${name}.bc)

    add_custom_command(
        OUTPUT ${bc}
        COMMAND ${M4_EXECUTABLE} ${input} | \"${LLVM_AS_EXECUTABLE}\" ${LLVM_TOOLS_OPAQUE_FLAGS} -o ${bc}
        DEPENDS ${input}
        WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
    )
...
function (generate_dispatch_builtins)
    dispatch_ll_to_cpp(dispatch "linux")
    dispatch_ll_to_cpp(dispatch-macos "macos")
