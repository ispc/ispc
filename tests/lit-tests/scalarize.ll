...
; CHECK-NEXT:  %3 = shufflevector <4 x i32> %2, <4 x i32> poison, <4 x i32> zeroinitializer
; CHECK-NEXT:  ret <4 x i32> %3
define <4 x i32> @test_poison(i32 %a) {
  %op1 = insertelement <4 x i32> poison, i32 %a, i32 0
  %sum = add <4 x i32> %op1, <i32 1, i32 poison, i32 poison, i32 poison>
  %shuffled = shufflevector <4 x i32> %sum, <4 x i32> poison, <4 x i32> zeroinitializer
  ret <4 x i32> %shuffled
...
define <2 x float> @fmul(float %a, float %b) {
  %op1 = insertelement <4 x float> poison, float %a, i32 0
  %binop = fmul <4 x float> <float 1.0, float poison, float poison, float poison>, %op1
  %shuffled = shufflevector <4 x float> %binop, <4 x float> poison, <2 x i32> zeroinitializer
  ret <2 x float> %shuffled
}

; cmp not supported
; CHECK-LABEL: @cmp
; CHECK-NEXT:  %op1 = insertelement <4 x i32> poison, i32 %a, i32 0
define <4 x i1> @cmp(i32 %a) {
  %op1 = insertelement <4 x i32> poison, i32 %a, i32 0
  %binop = icmp eq <4 x i32> %op1, <i32 1, i32 poison, i32 poison, i32 poison>
  %shuffled = shufflevector <4 x i1> %binop, <4 x i1> poison, <4 x i32> zeroinitializer
  ret <4 x i1> %shuffled
}

; not supported any position of non-poison value except the first one
...
; CHECK-NEXT:  %op1 = insertelement <4 x i32> poison, i32 %a, i32 0
; CHECK-NEXT:  %op2 = insertelement <4 x i32> poison, i32 %b, i32 0
define <4 x i32> @nonconst(i32 %a, i32 %b) {
  %op1 = insertelement <4 x i32> poison, i32 %a, i32 0
  %op2 = insertelement <4 x i32> poison, i32 %b, i32 0
...
define <4 x i32> @phi(i32 %a, i32 %b, i32 %c) {
  ...
  block_a:
    %incoming_a = insertelement <4 x i32> poison, i32 %a, i32 0
    br label %exit

  block_b:
    %incoming_b = insertelement <4 x i32> poison, i32 %b, i32 0
    br label %exit
