...
int main(int argc, char *argv[]) {
  ...
  // init data
  constexpr int N = 67;
  struct alignas(4096) AlignedArray { int data[N]; } AX, AY, AZ;
  void *XData = AX.data;
  void *YData = AY.data;
  ...
  cm_result_check(device->CreateBufferSVM(N * sizeof(int), XData, CM_SVM_ACCESS_FLAG_DEFAULT, XBuff));
  cm_result_check(device->CreateBufferSVM(N * sizeof(int), YData, CM_SVM_ACCESS_FLAG_DEFAULT, YBuff));
  cm_result_check(device->CreateBufferSVM(N * sizeof(int), OUTData, CM_SVM_ACCESS_FLAG_DEFAULT, OUTBuff));
