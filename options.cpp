...
OptionParseResult ParseOptions(int Argc, char* Argv[]) {
    int argc;
    char *argv[128];
    // Also retrieve options from environment.
    ...
    for (int i = 1; i < argc; ++i) {
        if (!strcmp(argv[i], "--help"))
            return OPTION_PARSE_RESULT_REQUEST_USAGE;
        if (!strcmp(argv[i], "--help-dev"))
            return OPTION_PARSE_RESULT_REQUEST_DEV_USAGE;
            ...
            if (atoi(argv[i] + 13) == 64)
                ...
                fprintf(stderr, "Addressing width \"%s\" invalid--only 32 and "
                        "64 are allowed.\n", argv[i]+13);
                return OPTION_PARSE_RESULT_ERROR | OPTION_PARSE_RESULT_REQUEST_USAGE;
            }
        }
          ...
          g->hostStubFileName = argv[i];
            ...
            return OPTION_PARSE_RESULT_REQUEST_VERSION;
        }
        else if (argv[i][0] == '-') {
    ...
    return OPTION_PARSE_RESULT_SUCCESS;
}
