...
/** @brief Type representing a template non-typename type.

    'TemplateNonTypeParmType' should be resolved during template instantiation.
 */
class TemplateNonTypeParmType : public Type {
  public:
    TemplateNonTypeParmType(std::string, Variability v, const Type *t, bool ic, SourcePos pos);
  ...
  private:
    ...
    const bool isConst;
    const SourcePos pos;
    mutable const TemplateNonTypeParmType *asOtherConstType, *asUniformType, *asVaryingType;
};
...
template <> inline const TemplateNonTypeParmType *CastType(const Type *type) {
    if (type != nullptr && type->typeId == TEMPLATE_NONTYPE_PARM_TYPE)
        return (const TemplateNonTypeParmType *)type;
    else
        return nullptr;
}
