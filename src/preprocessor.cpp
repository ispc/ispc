...
int lExecPreprocessor(llvm::Module *module, const char *infilename, llvm::raw_string_ostream *ostream,
                      Globals::PreprocessorOutputType preprocessorOutputType) {
...
void lInitCPPBuffer(std::unique_ptr<CPPBuffer> &bufferCPP) {
...
void lParseCPPBuffer(std::unique_ptr<CPPBuffer> &bufferCPP) {
...
void lClearCPPBuffer(std::unique_ptr<CPPBuffer> &bufferCPP) {
    if (bufferCPP) {
        bufferCPP.reset();
    }
}
