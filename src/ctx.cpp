...
llvm::Function *lGetMatchedFuncByRetType(llvm::Type *type, std::vector<Symbol *> &candidates) {
    for (auto &candidate : candidates) {
        llvm::Function *f = candidate->function;
...
llvm::Value *FunctionEmitContext::VectorIntDivision(llvm::Instruction::BinaryOps inst, llvm::Value *v0, llvm::Value *v1,
    ...
    // Handle the masked division case using __idiv
    SetCurrentBasicBlock(callDivBlock);
    std::vector<Symbol *> candidates;
    // TODO!: register __idiv and __udiv functions in the builtins.h
    std::string funcName = inst == llvm::Instruction::SDiv ? builtin::__idiv : builtin::__udiv;
    if (inst == llvm::Instruction::SRem || inst == llvm::Instruction::URem) {
        funcName = inst == llvm::Instruction::SRem ? builtin::__irem : builtin::__urem;
    }
    m->symbolTable->LookupFunction(funcName.c_str(), &candidates);

    llvm::Function *match = lGetMatchedFuncByRetType(type, candidates);
    AssertPos(currentPos, match != nullptr && "No matching function found for __[i|u]div or _[i|u]rem");
