...
void Function::GenerateIR() const {
    ...
    // Skip internal version if: exported AND --no-internal-export-functions flag is set
    bool skipInternalVersion = type->IsExported() && !g->generateInternalExportFunctions;

    if (g->target->isXeTarget()) {
        // For Xe target we do not emit code for masked version of a function
        // if it is a kernel
        if (!type->IsISPCKernel() && !type->IsExternalOnly() && !skipInternalVersion) {
        ...
        // <target> suffix.
        if (!type->IsExternalOnly() && !skipInternalVersion &&
            !((type->IsExternC() || type->IsExternSYCL()) && g->mangleFunctionsWithTarget)) {
            llvm::TimeTraceScope TimeScope("emitCode", llvm::StringRef(sym->name));
            FunctionEmitContext ec(this, sym, function, firstStmtPos);
            emitCode(&ec, function, firstStmtPos);
        }
